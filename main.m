...
if params.sim.constraints.uni(1) == 1 && params.sim.constraints.uni(2) == 1 && params.sim.constraints.uni(3) == 0
...
[A_bilateral,A_unilateral,Hessian_bilateral,Hessian_unilateral] = constraint_derivatives(x,params);

%% solve for constraint forces, if any; compute accelerations;  populate the events vector
n_active_constraints = sum(params.sim.constraints.uni);  % number of active unilateral constraints
...
if n_active_constraints == 0  % if there are no constraints active, then there are no forces
    ...
    for ic=1:4    % cycle through all of the unilateral constraints
        if params.sim.constraints.uni(ic) == 1    % if a constraint is active, add to A and Adotqdot
            A = [A;A_unilateral(ic,:)];
            Adotqdot = [Adotqdot;q_dot'*Hessian_unilateral(:,:,ic)*q_dot];
        end
    end
    % now add in the bilateral constraint
    if params.sim.constraints.uni(1) == 1  % if the left foot constraint is active, add in that bilateral
        A = [A;A_bilateral(1,:)];
        Adotqdot = [Adotqdot;q_dot'*Hessian_bilateral(:,:,1)*q_dot];
    elseif params.sim.constraints.uni(2) == 1  % if the left foot was NOT active, but the right foot is, add in that bilateral
        A = [A;A_bilateral(2,:)];
        Adotqdot = [Adotqdot;q_dot'*Hessian_bilateral(:,:,2)*q_dot];
    end
...
if nargout>1  % create a 6x1 vector of forces for plotting 
...
for i1 = 1:length(ie)  % I'm not sure if ie is ever a vector ... just being sure!
    if params.sim.constraints.uni(ie(i1)) == 1  % if the event came from an active constraint
        params.sim.constraints.uni(ie(i1)) = 0; % then make the constraint inactive
    else    % the event came from an inactive constraint --> collision
        collision = 1;
        params.sim.constraints.uni(ie(i1)) = 1; % make the constraint active
